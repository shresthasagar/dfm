...
class UNetModelWrapper(UNetModel):
    def __init__(
        ...
        out_ch = out_channels if out_channels is not None else (dim[0] if not learn_sigma else dim[0] * 2)
...
class DDM(nn.Module):
    def __init__(self, unified_model: UNetModelWrapper, fm_model: UNetModelWrapper):
        super().__init__()
    ...
    def forward(self, t, x, y=None, *args, **kwargs):
        if y is None:
            return self.unified_model(t, x, y=y)
        else:
            return self.fm_model(t, x, y=y)

class InterpolantModelWrapper(UNetModelWrapper):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def forward(self, t, x0, x1, y=None, *args, **kwargs):
        return super().forward(t, th.cat([x0, x1], dim=1), y=y)
...
class SimpleMLP(nn.Module):
    ...
    def forward(self, t, x0, x1, labels):
        x0 = x0.view(x0.size(0), -1)
        x1 = x1.view(x1.size(0), -1)
        labels = labels.float().view(labels.size(0), -1)
        t = t.view(t.size(0), -1)
        
        print(t.shape, x0.shape, x1.shape, labels.shape)
        x = th.cat([t, x0[:, :2], x1[:, :2], labels], dim=1)
